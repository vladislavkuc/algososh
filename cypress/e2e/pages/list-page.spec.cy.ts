describe('list page works correctly', () => {
  ...
  it('all buttons disabled states are ok', function() {
    ...
    cy.get('button').eq(4).should('not.be.disabled');
    cy.get('button').eq(5).should('be.disabled');
    cy.get('button').eq(6).should('not.be.disabled');
  ...
  it('default list is ok', function() {
    ...
    cy.get('[class*="circle_letter"]').eq(0).should('contain', '0');
    cy.get('[class*="circle_letter"]').eq(1).should('contain', '34');
    cy.get('[class*="circle_letter"]').eq(2).should('contain', '8');
    cy.get('[class*="circle_letter"]').eq(3).should('contain', '1');
  });
